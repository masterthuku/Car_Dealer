...
function EmiCalculator({ price = 1000 }) {
  ...
  const handleLoanAmountChange = (value) => {
    ...
    const newLoanAmount = isNaN(parsedValue) ? loanAmount : Math.min(Math.max(parsedValue, 1000), 600000);
  ...
  const handleDownPaymentChange = (value) => {
    const parsedValue = parseFloat(value);
    const newDownPayment = isNaN(parsedValue) ? downPayment : Math.min(Math.max(parsedValue, 0), loanAmount);
    setDownPayment(newDownPayment);
  ...
  const handleDownPaymentPercentChange = (percent) => {
    ...
    const newPercent = isNaN(parsedPercent) ? downPaymentPercent : Math.min(Math.max(parsedPercent, 0), 100);
  ...
  const handleInterestRateChange = (value) => {
    const parsedValue = parseFloat(value);
    const newRate = isNaN(parsedValue) ? interestRate : Math.min(Math.max(parsedValue, 0.1), 25);
  ...
  const handleLoanTenureChange = (value) => {
    const parsedValue = parseFloat(value);
    const newTenure = isNaN(parsedValue) ? loanTenure : Math.min(Math.max(parsedValue, 1), 8);
            ...
            <div className="space-y-3">
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <span className="text-gray-700 dark:text-gray-300 mr-3">KES</span>
                </div>
                <input
                  type="number"
                  value={loanAmount}
                  onChange={(e) => handleLoanAmountChange(e.target.value)}
                  className="w-full pl-12 pr-4 py-2 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:border-gray-900 dark:focus:border-gray-400"
                />
              </div>
              <input
                type="range"
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <span className="text-gray-700 dark:text-gray-300">Years</span>
                  </div>
                </div>
                <input
                  type="range"
                  ...
                  max="8"
                  value={loanTenure}
                  onChange={(e) => handleLoanTenureChange(parseFloat(e.target.value))}
                  className="w-full"
                />
              </div>
            </div>
