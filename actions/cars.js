...
async function fileToBase64(file) {
  ...
  return bufffer.toString("base64");
}
  ...
  `;
    ...
    try {
      ...
      if (missingFields.length > 0) {
        throw new Error(
          `AI response is missing the following fields: ${missingFields.join(
            ", "
          )}`
        );
      }
...
export async function getCars(search = "") {
  try {
    ...
    const serialilizedCars = cars.map(serializedCarData);

    return {
      success: true,
      data: serialilizedCars,
    };
  } catch (error) {
